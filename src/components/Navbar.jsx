  ...
  return (
    <header className="glass-panel" style={{
      ...
      <div onClick={() => setCurrentTab('dashboard')} 
        ...
        <div style={{ width: '34px', height: '34px', borderRadius: 'var(--radius-sm)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <button onClick={onOpenQuickAction} className="btn-touch btn-primary"
          style={{ padding: '6px 12px', borderRadius: 'var(--radius-sm)', fontSize: '0.825rem', minHeight: '34px' }}>
          <Plus size={15} />
        ...
        <div onClick={() => setCurrentTab('profile')} 
          style={{ padding: '4px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-glass)',
            cursor: 'pointer', background: currentTab === 'profile' ? 'var(--glass-hover)' : 'transparent' }}>
          <img src={profile.avatar} alt={profile.name} 
            style={{ width: '26px', height: '26px', borderRadius: 'var(--radius-sm)', objectFit: 'cover', display: 'block' }} />
        </div>
      </div>
